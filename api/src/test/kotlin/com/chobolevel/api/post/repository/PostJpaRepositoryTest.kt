...
@ActiveProfiles("test")
@DisplayName("PostJpaRepository 슬라이스 테스트")
class PostJpaRepositoryTest {
    ...
    fun `삭제되지 않은 게시글을 id로 조회하면 게시글을 반환한다`() {
        ...
        val savedPost: Post = savedPost(user)
        entityManager.clear()

        // when
        val result: Post? = postJpaRepository.findByIdAndDeletedFalse(savedPost.id!!)
    ...
    fun `QueryDSL로 집계하면 삭제되지 않은 게시글 수만 반환한다`() {
        ...
        // when
        val count: Long = postQuerydslRepository.countByPredicates(
            predicates = queryFilter.toPredicates()
        )
