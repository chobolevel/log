...
@ActiveProfiles("test")
@DisplayName("ChannelMessageJpaRepository 슬라이스 테스트")
class ChannelMessageJpaRepositoryTest {
    ...
    fun `삭제되지 않은 메시지를 id로 조회하면 메시지를 반환한다`() {
        ...
        val channel: Channel = savedChannel(user)
        val msg: ChannelMessage = savedChannelMessage(channel, user)
        entityManager.clear()
    ...
    fun `QueryDSL로 channelId 필터를 적용하면 해당 채널의 메시지만 조회한다`() {
        ...
        // ChannelMessageQuerydslRepository.searchByPredicates는 결과를 reversed()한다.
        // 최신 메시지가 앞에 오도록 desc로 정렬 후 뒤집으면 오래된 메시지가 앞에 온다.
