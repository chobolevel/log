...
@WebMvcTest(RecordLikeSyncEventController::class)
@Import(RecordLikeSyncEventControllerTest.TestSecurityConfig::class)
@ActiveProfiles("test")
@DisplayName("RecordLikeSyncEventController 슬라이스 테스트")
class RecordLikeSyncEventControllerTest {

    @Autowired
    private lateinit var mockMvc: MockMvc

    @MockkBean
    private lateinit var recordLikeSyncEventService: RecordLikeSyncEventService
    ...
    fun `인증된 관리자가 실패한 이벤트 목록 조회 요청 시 목록을 반환한다`() {
        // given
        every {
            recordLikeSyncEventService.searchFailedEvents(request = any())
        } returns PagingResponse(
            page = 1L,
            size = 100L,
            data = listOf<RecordLikeSyncEventResponse>(DummyRecordLikeSyncEvent.toResponse()),
            totalCount = 1L
        )
    ...
    fun `인증된 관리자가 이벤트 재발행 요청 시 이벤트 id를 반환한다`() {
        // given
        every {
            recordLikeSyncEventService.retry(eventId = DummyRecordLikeSyncEvent.ID)
        } returns DummyRecordLikeSyncEvent.ID
        ...
        mockMvc.perform(
            ...
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.data").value(DummyRecordLikeSyncEvent.ID))
    }
